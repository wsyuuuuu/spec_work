...
{{~end~}}
    ...
    public {{name}}(ByteBuf _buf)
    {
        ...
        {{~ for field in value_type.bean.hierarchy_export_fields ~}}
         { _data.{{field.convention_name}}_l10n_key, _data.{{field.convention_name}} },
        {{~end~}}
        };
        PostInit();
    }
...
{{~end~}}
    ...
    {{~end~}}
     ...
     public string this[string key] => _textMap[key];
     public string Get(string key) => _textMap.TryGetValue(key, out string result) ? result : $"[Missing Text] {key}";
     public Dictionary<string, string>.KeyCollection Keys => _textMap.Keys;
     public Dictionary<string, string>.ValueCollection Values => _textMap.Values;
     public Dictionary<string, string>.Enumerator GetEnumerator() => _textMap.GetEnumerator();
